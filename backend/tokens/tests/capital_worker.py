...
def run(directory, phase, request_id, actor_id, confirmation):
    ...
    database = json.loads(os.environ["CAPITAL_TEST_DATABASE"])
    ...
    def observed(tx_hash):
        ...
        command = CapitalIncreaseExecution.objects.get(operation=attempt.operation)
    ...
    def checked(*args, **kwargs):
        try:
            ...
            if (
                phase == "attributed"
                and CapitalIncreaseExecution.objects.get(pk=request.dispatch_id).attribution_evidence
            ):
                os.kill(os.getpid(), signal.SIGKILL)
