...
def run(directory, phase, submission_id, new_id=None):
    ...
    if database["ENGINE"] != "django.db.backends.postgresql":
        raise RuntimeError("NAV crash evidence requires real PostgreSQL")
    ...
    from tokens.tests.nav_fixtures import NAVNode

    command = NAVUpdate.objects.get(pk=submission_id)
    node = NAVNode(command.intent["contract_address"])
    ...
    with ExitStack() as stack:
        stack.enter_context(patch.object(nav_recovery, "get_base_chain_client", return_value=node.client))
        ...
        if phase == "admit":
            result = nav.submit(command.yield_token, command.updated_by, new_id, "2", "600", update_on_chain=True)
        else:
            result = nav_recovery.recover(submission_id)
