...
| `classes/<class id>/chain.json` | The class's deployment record, and every transaction Ledova's operator prepared or signed for the class: its purpose, the unsigned transaction, its status and receipt, and each signed attempt's hash, nonce, signer and chain id |
