...
class CorrectionReviewForm(forms.Form):
    ...
    reviewed = forms.BooleanField(
        required=False,
        label="I verified the named authority and its documentary approval of this exact register correction.",
    )
...
class RegisterCorrectionAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        return [
            admin_action_path(self, "<uuid:uuid>/review/", "tokens_registercorrection_review", self.review),
    ...
    def review_link(self, obj):
        ...
        return format_html(
            '<a href="{}">Review correction</a>', reverse("admin:tokens_registercorrection_review", args=[obj.pk])
        )

    @method_decorator(require_http_methods(["GET", "POST"]))
    def review(self, request, proposal):
        form = CorrectionReviewForm(request.POST if request.method == "POST" else None)
        refusal = ""
        try:
            if request.method == "GET":
                proposal, form.initial["confirmation"] = prepare_correction_review(
                ...
                self.log_change(request, proposal, f"Register correction {proposal.status}.")
                self.message_user(request, f"Register correction {proposal.status}.", messages.SUCCESS)
                return redirect("admin:tokens_registercorrection_change", proposal.pk)
        except RegisterChangeConflict:
            refusal = "This correction conflicts with the current register or an existing decision."
        except ValidationError as error:
            refusal = " ".join(str(item) for item in error.detail)
        return render(
            request,
            "admin/tokens/register_correction_review.html",
            {
                **self.admin_site.each_context(request),
                "opts": self.model._meta,
                "original": proposal,
                "title": "Review register correction",
                "form": form,
                "refusal": refusal,
