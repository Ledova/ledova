...
it('retains the selected Base network after entering and scanning an EVM address', async () => {
  ...
  const { result } = await renderHook(() => useAddWalletForm({ onSubmit: submit, onBatchSubmit: jest.fn() }));
  ...
  expect(submit).toHaveBeenLastCalledWith(expect.objectContaining({ chain: 'base' }));
  await act(async () => result.current!.handleQRScan('0x' + 'b'.repeat(40)));
  await act(async () => result.current!.handleSubmit());
  expect(submit).toHaveBeenLastCalledWith(expect.objectContaining({ chain: 'base', address: '0x' + 'b'.repeat(40) }));
});
