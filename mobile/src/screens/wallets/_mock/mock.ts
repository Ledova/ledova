...
export const generateMockWalletsData = (): Wallet[] => {
  ...
  for (let i = 0; i < MOCK_WALLETS_PER_CHAIN; i++) {
    ...
    wallets.push({
      ...
      verifiedAt: status === 'VERIFIED' ? new Date(Date.now() - (i + 1) * 2 * 24 * 60 * 60 * 1000).toISOString() : null,
      verificationChallenge: status === 'PENDING' ? `verify-btc-${i}` : null,
  ...
  for (let i = 0; i < MOCK_WALLETS_PER_CHAIN; i++) {
    ...
    wallets.push({
      ...
      verificationStatus: status,
      verificationSignature: null,
      verifiedAt: status === 'VERIFIED' ? new Date(Date.now() - (i + 1) * 3 * 24 * 60 * 60 * 1000).toISOString() : null,
      verificationChallenge: status === 'PENDING' ? `verify-eth-${i}` : null,
      nativeBalance: balance,
