...
`backend/schema/openapi.json` records the complete OpenAPI document. Generation
uses Python 3.13.15, PostgreSQL 16, the credential-free PostgreSQL test settings
and the existing schema-producing packages constrained in
`backend/schema/requirements.txt`. In an isolated development environment, run
`make install-schema-environment`, migrate the dedicated PostgreSQL database
with `ledova_backend.settings.test_postgres`, then run `make check-api-schema`.
The `POSTGRES_*` variables select that database. The generator refuses another
...
Ordinary comparison never rewrites a missing or stale snapshot. Canonicalisation
sorts mapping keys only; array order, numeric bounds, required/write-only fields,
nullability, response media types and event metadata all remain in the comparison.
CI retains the generated document, environment, diagnostics and drift report.

Registered-operation coverage uses the same route walker as the tenant route
matrix, with its existing administrative/static/API-root/format-duplicate and
bodyless-method scope. The only schema exclusions are the Alchemy, KYCAID identity,
KYCAID crypto and Sumsub provider webhooks. An unlisted route, a phantom schema
operation, a stale exclusion or a provider webhook newly exposed in the schema
fails. Currently unused application routes and the trading stream remain included.
An error-only compatibility route still counts as a registered operation.

`make check-client-operations` separately inspects shared, dashboard and mobile
production TypeScript with the locked compiler and Axios declarations. It checks
method/path presence and a successful response kind, including untyped 204,
scalar, text, binary and SSE replies. It recognises Axios by its declaration,
not the spelling of a `.get` call, so Map, headers and file operations are not
HTTP requests. Unresolved destinations/configurations, indirect Axios method
references and unclassified fetch/stream transports fail with a source location;
new call shapes need explicit tested accounting. This is operation coverage,
not a replacement for TypeScript checking or nested schema validation.

The two API-client interceptors replay the original `AxiosError.config`; those
sites are recorded separately and cannot change its destination, method or origin.
The mobile company-document download follows a response `fileUrl`. Its explicit
mechanism checks the registered stored-file route and binary response metadata;
link-backed documents may supply an external URL, which is not a registered API
operation. This census does not change or certify external destination policy.
The browser stream's configured-origin template and mobile stream URL builder
