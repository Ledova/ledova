...
any id the unlabelled suite finds more than once. It compares ids, not classes: a
factory that caches its class by name builds it once, for whichever module calls
it first, so an unassigned app's copy can share an id with the class an assigned
app's shard builds, and the checker passes. A module that fails to import is a
finding rather than a module: it is discovered as one `_FailedTest` on both sides
and would otherwise look covered.
